...
describe("SubGrid", function () {
    describe("2x4SubGrid", function () {
        it("should have a 2 x 4 sub grid created", function () {
            ...
            var subGrid: Solver.ISubGrid = new Solver.SubGrid(0, 0);

            // Ensure a 2 x 4 sub grid created
            var expectedCells: Solver.ICell[][] = [];
    ...
    describe("4x2SubGrid", function () {
        ...
        it("should be solved", function () {
            subGrid.setByPosition(0, 0, 0, 0, Solver.SetMethod.user);
            subGrid.setByOption(1, 0, 2, Solver.SetMethod.user);
            subGrid.setByPosition(2, 0, 2, 0, Solver.SetMethod.loaded);
            subGrid.setByOption(3, 0, 4, Solver.SetMethod.loaded);
            subGrid.setByOption(0, 1, 8, Solver.SetMethod.loaded);
            subGrid.setByPosition(1, 1, 1, 1, Solver.SetMethod.loaded);
            subGrid.setByOption(2, 1, 32, Solver.SetMethod.user);
            subGrid.setByPosition(3, 1, 3, 1, Solver.SetMethod.user);
    ...
    describe("Remove options", function () {
        ...
        it("should have 1 removed from all cells except top left cell", function () {
            ...
            var removeBit: number = 1;
            var testColumn: number = 0;
            var testRow: number = 0;

            var struckOutCells: Solver.IStruckOutCells = subGrid.strikeOutCell(testColumn, testRow, removeBit);
            expect(struckOutCells.lastOptionsFound.length).toBe(0);                                 // No last options found

            var removeFromColumn: Solver.IOption[] = struckOutCells.removedOptionsFromColumn;       // Remove 1 from columns 2 and 1
            ...
            var removeFromRow: Solver.IOption[] = struckOutCells.removedOptionsFromRow;             // Remove 1 from row 1
        ...
        it("should also have 2 removed from all cells except top middle cell", function () {
            var removeBit: number = 2;
            var testColumn: number = 1;
            var testRow: number = 0;
            ...
            var removeFromColumn: Solver.IOption[] = struckOutCells.removedOptionsFromColumn;       // Remove 2 from columns 2 and 0
            expect(removeFromColumn.length).toBe(2);
            expect(removeFromColumn[0].cellColumn).toBe(2);
            expect(removeFromColumn[0].bits).toBe(removeBit);
            ...
            var removeFromRow: Solver.IOption[] = struckOutCells.removedOptionsFromRow;             // Remove 2 from row 1
            expect(removeFromRow.length).toBe(1);
        ...
        it("should have 4 removed from all cells except top right cell", function () {
            ...
            var removeBit: number = 4;
            var testColumn: number = 2;
            var testRow: number = 0;

            var struckOutCells: Solver.IStruckOutCells = subGrid.strikeOutCell(testColumn, testRow, removeBit);
            expect(struckOutCells.lastOptionsFound.length).toBe(0);                                 // No last options found

            var removeFromColumn: Solver.IOption[] = struckOutCells.removedOptionsFromColumn;       // Remove 4 from columns 1 and 0
            expect(removeFromColumn.length).toBe(2);
            ...
            var removeFromRow: Solver.IOption[] = struckOutCells.removedOptionsFromRow;             // Remove 4 from row 1
            expect(removeFromRow.length).toBe(1);
            expect(removeFromRow[0].cellRow).toBe(1);
            expect(removeFromColumn[0].bits).toBe(removeBit);
        ...
        it("should have 8 removed from all cells except bottom left cell", function () {
            ...
            var removeBit: number = 8;
            var testColumn: number = 0;
            var testRow: number = 1;

            var struckOutCells: Solver.IStruckOutCells = subGrid.strikeOutCell(testColumn, testRow, removeBit);
            expect(struckOutCells.lastOptionsFound.length).toBe(0);                                 // No last options found

            var removeFromColumn: Solver.IOption[] = struckOutCells.removedOptionsFromColumn;       // Remove 8 from columns 2 and 1
            expect(removeFromColumn.length).toBe(2);
            expect(removeFromColumn[0].cellColumn).toBe(2);
            expect(removeFromColumn[0].bits).toBe(removeBit);
            expect(removeFromColumn[1].cellColumn).toBe(1);
            ...
            var removeFromRow: Solver.IOption[] = struckOutCells.removedOptionsFromRow;             // Remove 8 from row 0
            expect(removeFromRow.length).toBe(1);
        ...
        it("should have 16 removed from all cells except bottom middle cell", function () {
            ...
            var removeBit: number = 16;
            var testColumn: number = 1;
            var testRow: number = 1;

            var struckOutCells: Solver.IStruckOutCells = subGrid.strikeOutCell(testColumn, testRow, removeBit);
            var lastOptions: Solver.IOption[] = struckOutCells.lastOptionsFound;
            ...
            var removeFromColumn: Solver.IOption[] = struckOutCells.removedOptionsFromColumn;       // Remove 16 from columns 2 and 0
            expect(removeFromColumn.length).toBe(2);
            expect(removeFromColumn[0].cellColumn).toBe(2);
            ...
            var removeFromRow: Solver.IOption[] = struckOutCells.removedOptionsFromRow;             // Remove 16 from row 0
            expect(removeFromRow.length).toBe(1);
            expect(removeFromRow[0].cellRow).toBe(0);
            expect(removeFromColumn[0].bits).toBe(removeBit);

            for (var row: number = 0; row < rows; row++)                                            //  33 | 34 | 36        (1,6) | (2,6) | (3,6)
                for (var column: number = 0; column < columns; column++)                            //  ------------    =   --------------------- 
        ...
        it("should be solved", function () {
            ...
            expectedCells[0][0].setByPosition(0, 0, Solver.SetMethod.user);                         // Cells transposed to sub-grid i.e. n x m -> m x n
            expectedCells[0][1].setByPosition(1, 0, Solver.SetMethod.user);
            expectedCells[0][2].setByPosition(0, 1, Solver.SetMethod.user);
            expectedCells[1][0].setByPosition(1, 1, Solver.SetMethod.user);
            expectedCells[1][1].setByPosition(0, 2, Solver.SetMethod.user);
            expectedCells[1][2].setByPosition(1, 2, Solver.SetMethod.user);
            expect(subGrid.compare(expectedCells)).toBeTruthy();
        });
    });
    ...
    describe("2x2SubGridCells", function () {
        it("should be solved", function () {
            ...
            subGrid.setByPosition(0, 0, 0, 0, Solver.SetMethod.user);                               // Top left cell set to 1
            subGrid.setByPosition(1, 0, 1, 0, Solver.SetMethod.user);                               // Top right cell set to 2
            subGrid.setByPosition(0, 1, 0, 1, Solver.SetMethod.user);                               // Bottom left cell set to 4
...
//            expect('ToDo').toBe('Not Finished yet');
            ...
            subGrid.get(0, 0).setByPosition(0, 0, Solver.SetMethod.loaded);                         // Top left cell set to 1
    ...
    describe("Json", function () {
        describe("1x2", function () {
            ...
            it("should have cell set", function () {
                subGrid.setByOption(0, 0, 1, Solver.SetMethod.user);
                expect(subGrid.toJson()).toEqual({
                    rows:
                    [
        ...
        describe("2x2", function () {
            ...
            it("should have top left cell set", function () {                                       // Other cells not struck out
                subGrid.setByPosition(0, 0, 0, 0, Solver.SetMethod.user);
            ...
            it("should have other cells set", function () {
                subGrid.setByOption(1, 0, 2, Solver.SetMethod.loaded);
                subGrid.setByPosition(0, 1, 0, 1, Solver.SetMethod.loaded);
                expect(subGrid.toJson()).toEqual({
                    rows:
                    [
                        {
